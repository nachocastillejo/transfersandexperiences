...
def create_app():
    ...
    # Importar blueprints después de cargar la configuración
    from .views import webhook_blueprint
    
    # Inicializa la integración con Slack si está habilitada
    # Importamos slack_utils aquí (lazy) para no crashear si la librería no está instalada
    if app.config.get('ENABLE_SLACK'):
        try:
            ...
            app.logger.warning(f"Slack desactivado: librerías no instaladas ({e}).")
    else:
